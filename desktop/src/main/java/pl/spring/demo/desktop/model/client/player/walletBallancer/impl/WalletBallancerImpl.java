...
public class WalletBallancerImpl implements WalletBalancer{
	...
	public double getBallance(Currency primary, double quantA, Currency foregin, double quantB) {
		...
		logger.info("primary: "+DoubleRounder.roundToMoney(valuePrimary)+" foreign: "+DoubleRounder.roundToMoney(quantB));
		return ballance;
		}

			private ExchangeTransaction createBuyForeginCurrencyToBackBallanced(double quantA, Currency foreign, double quantB){
			...
			private ExchangeTransaction createSellForeginCurrencyToBackBallanced(double quantA, Currency foreign, double quantB){
			double valueOfWallet=getaValueOfWalletInPrimaryCurrency(quantA, foreign, quantB);
			double ballanced=getDesiredValueOfPrimary(valueOfWallet);
